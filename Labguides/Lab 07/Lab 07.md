...
1. In the Copilot Studio page, Select the **DevOne**  environment from the top right.
...
1. Once the agent is created, select **Edit** against the **Details**.
...
1. Search and select **When a new email arrives (V3) Office 365 Outlook** and click on **Next**. 

    ![](./media/image6.png)

1. Rename the trigger to +++When a portfolio lookup email arrives+++,
ensure that the connection is established for Copilot Studio and Outlook and then click on **Next**.

    ![](./media/image7.png)

1. In the **Subject Filter (Optional)** field, enter +++Portfolio+++ in the subject line.

    ![](./media/image8.png)

1. Once the trigger is created, you can **Close** the Time to test your trigger dialog.
...
1. Select **Computer use (preview)**.

    ![](./media/image13.png)

1. Add the following Instructions, and then select **Add and configure**.
...
1. Enter name as **Portfolio ID** and description **The ID of the portfolio** and select **Done**.
...
1. Update its **Name** to +++Reply to email+++ and **Description** to,
**Use this operation to reply to the email received** and then select **Additional details**.

    ![](./media/image31.png)

1. Under **Additional details**, set **Credentials to use** to
    **Maker-provided credentials.**

    ![](./media/image32.png)

1. Under the **Inputs** section, click on **customize** against the **To** input and set its **Description** to +++Use the "from" email of the triggering received email+++.

    ![](./media/image33.png)
    
    ![](./media/image34.png)

1. **Customize** the **Subject** input and set its **Description** to **Write the email subject**.

    ![](./media/image35.png)

1. Customize the **Body** input and set its **Description** to +++Write the email body using HTML and highlight the requested data+++.
...
1. Paste the following instruction.
...
1. Select < Look up portfolio data >, enter / and select the **tool**
...
1. Similarly, replace < Reply to email > with the **tool**, **Reply to
    email**.

1. Once the replacements are done, as in the screenshot below, select
    **Save**.
...
1. **Disable** **Use general knowledge option** under the **Knowledge**
...
1. Make sure you receive the email in your training user’s inbox.
...
1. The execution happens and you can see the updates and the flow in the Test pane.

    ![](./media/image49.png)

    ![](./media/image50.png)

1. Once the execution is completed, check your email for the agent’s reply.
